...
public class MemberDAL extends ConnectData {
    ...
    private final String SUM_POINT = "sumPoint";
    ...
    public ArrayList<Member> sumPoint() {
        try {
            getConnect();
            ArrayList<Member> all = new ArrayList<>();
            ps = con.prepareStatement(SUM_POINT);
            ResultSet rs = ps.executeQuery();
            if (rs != null) {
                while (rs.next()) {
                    Member mem = new Member();
